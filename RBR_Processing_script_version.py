...
# 5 lines of processing comments are added here and included in the write_comments function.
# If more lines are needed, adjust the function.
...
READ_RSK(dest_dir, year, cruise_number,
         skipcasts, meta_dict, zoh, fix_spk, fill_action,
         fill_type, spk_window, spk_std, spk_var,
...
cast_d_dropvars.pop('cast918')

cast_d_binned = BINAVE(cast_d_dropvars, metadata_dict=meta_dict)
...
for i in cast_numbers:
    ...
    main_header2(
        dest_dir, i, meta_dict, cast, cast_d_final, dict_recs_out, channel_names,
        processing_report_name, processing_comments1
    )
